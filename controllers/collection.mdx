...
`CollectionController` is the concern responsible for loading and shaping the list of records shown in admin index pages and similar collection-backed views. Rather than implementing everything in one place, it is itself composed of four narrowly focused sub-concerns — `FilterController`, `ScopeController`, `SortController`, and `PaginationController` — each of which hooks into the same `load_collection` pipeline via Ruby's `super` chaining mechanism.
