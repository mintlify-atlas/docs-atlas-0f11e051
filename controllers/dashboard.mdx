...
<Note>
If no `dashboard` route has been declared by the time routes are finalised, Torque Admin automatically adds one via `finalize_routes!`. Internally, `Application#finalize_routes!` calls `auto_dashboard_route`, which runs `dashboard_root` through a new `Mapper` instance. `dashboard_root` only registers the dashboard when no `dashboard` named route already exists, so it is safe to call even if you have declared one manually. This ensures every admin application always has a working home page.
</Note>
