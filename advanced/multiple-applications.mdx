---
title: "Multiple Admin Applications in a Single Rails App"
...
<Note>
Each application's `mod.name` is used as the Rails layout name. For the `:admin` application the layout must be `app/views/layouts/admin.html.erb`; for `:partner` it must be `app/views/layouts/partner.html.erb`. Ensure these layout files exist and match the application's module name exactly.
</Note>
